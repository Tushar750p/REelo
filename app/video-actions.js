(function(){
  ...
  const sendPreference=(id,action)=>{const t=localStorage.getItem('reelo_token')||'';if(!t||!id)return;try{fetch((window.REELO_API||'http://localhost:8000')+'/api/recommendations/feedback',{method:'POST',headers:{'Content-Type':'application/json','Authorization':'Bearer '+t},body:JSON.stringify({video_id:id,action,seconds:0}),keepalive:true}).catch(()=>{});}catch(e){}};
  function ensureSheet(){
    if(document.getElementById('videoActions'))return document.getElementById('videoActions');
    const el=document.createElement('section');el.id='videoActions';el.className='video-actions';
    ...
    el.addEventListener('click',e=>{if(e.target===el)closeActions()});el.querySelector('.back').addEventListener('click',closeActions);
    document.querySelector('.shell')?.appendChild(el);return el;
  }
  ...
  function openActions(id){
    ...
    const v=findVideo(id);state={id:String(id),userId:v?.user_id||null,username:v?.username||v?.display_name||'creator'};
    const sheet=ensureSheet(),box=document.getElementById('videoActionButtons'),creator=state.userId?String(state.userId):'',muted=creator&&readSet(MUTED_CREATORS).has(creator);
  ...
  function notInterested(){
    const s=readSet(NOT_INTERESTED);if(s.has(state.id)){closeActions();return;}s.add(state.id);writeSet(NOT_INTERESTED,s);sendPreference(state.id,'not_interested');
    const id=state.id;closeActions();document.querySelector('[data-id="'+CSS.escape(id)+'"]')?.remove();toast('We’ll show fewer videos like this');
  }
  function toggleMute(){
    const uid=state.userId;if(!uid)return;const s=readSet(MUTED_CREATORS),muted=s.has(String(uid));
    if(muted)s.delete(String(uid));else{s.add(String(uid));sendPreference(state.id,'mute_creator');}
    writeSet(MUTED_CREATORS,s);const name=state.username;closeActions();applyFilters();toast(muted?'@'+name+' unmuted':'@'+name+' muted');
  }
  ...
  function applyFilters(){
    ...
    document.querySelectorAll('[data-id]').forEach(el=>{const id=el.getAttribute('data-id'),v=findVideo(id);if(v&&(hidden.has(String(id))||(v.user_id&&muted.has(String(v.user_id)))))el.remove()});
  }
  document.addEventListener('click',function(e){
    const btn=e.target.closest('.more');if(!btn)return;e.preventDefault();e.stopPropagation();e.stopImmediatePropagation();
    const m=(btn.getAttribute('onclick')||'').match(/openReport\\(['\"]([^'\"]+)['\"]\\)/);if(m)openActions(m[1]);
  },true);
  const timer=setInterval(applyFilters,1200);window.addEventListener('beforeunload',()=>clearInterval(timer));
})();
