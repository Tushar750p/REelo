...
def recommendations(limit: int = 20, offset: int = 0, authorization: str | None = Header(default=None)):
    ...
    with db() as c:
        ...
        rows = c.execute("""
            SELECT v.*, u.username, u.display_name,
                   ...
                   COALESCE((SELECT SUM(CASE
                       ...
                       WHEN e.action IN ('like','comment','share','save') THEN 2.0 ELSE 0 END)
                       FROM events e WHERE e.user_id=? AND e.video_id=v.id),0) AS watch_score,
                   COALESCE((SELECT SUM(CASE
                       WHEN e.action IN ('watch','complete') THEN MIN(COALESCE(e.seconds,0),30.0)/30.0
                       WHEN e.action IN ('like','comment','share','save') THEN 1.5
                       WHEN e.action IN ('skip','dismiss') THEN -0.75 ELSE 0 END)
                       FROM events e JOIN videos hv ON hv.id=e.video_id
                       WHERE e.user_id=? AND hv.user_id=v.user_id AND hv.id<>v.id),0) AS creator_affinity,
                   CASE WHEN EXISTS(SELECT 1 FROM events e2 WHERE e2.user_id=? AND e2.video_id=v.id) THEN 1 ELSE 0 END AS seen,
                   CASE WHEN EXISTS(SELECT 1 FROM events e3 JOIN videos rv ON rv.id=e3.video_id
                       WHERE e3.user_id=? AND rv.user_id=v.user_id AND e3.created_at>=datetime('now','-24 hours')) THEN 1 ELSE 0 END AS recent_creator,
                   COALESCE((SELECT SUM(CASE WHEN e.action IN ('not_interested','skip','dismiss') THEN -1.0 ELSE 0 END)
                       FROM events e WHERE e.user_id=? AND e.video_id=v.id AND e.created_at>=datetime('now','-90 days')),0) AS negative_feedback,
                   CASE WHEN EXISTS(SELECT 1 FROM events en WHERE en.user_id=? AND en.video_id=v.id
                       AND en.action IN ('not_interested','skip','dismiss') AND en.created_at>=datetime('now','7 days')) THEN 1 ELSE 0 END AS recent_negative
            ...
            WHERE v.status='ready' AND v.user_id<>?
        ...
        history = c.execute("""
            SELECT v.*, e.action,
                   CASE WHEN e.action IN ('watch','complete','like','comment','share','save') THEN 1
                        WHEN e.action IN ('not_interested','skip','dismiss','mute_creator') THEN -1 ELSE 0 END AS signal
    ...
    for row in history:
        for topic in infer_topics(_content_text(dict(row))):
            topic_profile[topic] = topic_profile.get(topic,0.0) + float(row['signal'] or 0)

    videos=[]; signals={}
    for raw in rows:
        video=dict(raw); vid=str(video['id'])
        topics=infer_topics(_content_text(video)); video['_topics']=topics
        signals[vid]={
            ...
            'topic_affinity':sum(topic_profile.get(t,0.0) for t in topics),
            'seen':bool(video.pop('seen')),
            'recent_creator':bool(video.pop('recent_creator')),
            'negative_feedback':float(video.pop('negative_feedback') or 0),
            'recent_negative':bool(video.pop('recent_negative')),
        }
        videos.append(video)

    # Current-session signals are derived from the most recent observed actions.
    ...
    for row in history[:50]:
        ...
        if int(d.get('signal') or 0)>0:
            for topic in infer_topics(_content_text(d)):
                session_topics[topic]=session_topics.get(topic,0.0)+0.35

    ranked=rank_videos(videos,signals,session_creator_counts,session_topics)
    page=ranked[offset:offset+limit]
    return {'items':page,'personalized':True,'limit':limit,'offset':offset,
