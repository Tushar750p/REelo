...
def recommendations(limit: int = 20, offset: int = 0, authorization: str | None = Header(default=None)):
    ...
    with db() as c:
        ...
        COALESCE((SELECT SUM(CASE WHEN e.action IN ('watch','complete') THEN CASE WHEN COALESCE(e.seconds,0)>30.0 THEN 30.0 ELSE COALESCE(e.seconds,0) END/6.0 WHEN e.action IN ('skip','dismiss') THEN -2.5 WHEN e.action IN ('like','comment','share','save') THEN 2.0 ELSE 0 END) FROM events e WHERE e.user_id=? AND e.video_id=v.id),0) watch_score,
        COALESCE((SELECT SUM(CASE WHEN e.action IN ('watch','complete') THEN CASE WHEN COALESCE(e.seconds,0)>30.0 THEN 30.0 ELSE COALESCE(e.seconds,0) END/30.0 WHEN e.action IN ('like','comment','share','save') THEN 1.5 WHEN e.action IN ('skip','dismiss') THEN -0.75 ELSE 0 END) FROM events e JOIN videos hv ON hv.id=e.video_id WHERE e.user_id=? AND hv.user_id=v.user_id AND hv.id<>v.id),0) creator_affinity,
