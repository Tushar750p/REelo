...
def collection_detail(collection_id: str, authorization: str | None = Header(default=None)):
    uid = require_user(authorization)
    with db() as c:
        ...
        """, (uid, collection_id)).fetchall()
    return {"collection": dict(col), "items": [dict(r) for r in rows]}
