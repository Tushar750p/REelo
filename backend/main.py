...
def current_user(a):
    ...
    try:
        p,s=a.split(" ",1)[1].split(".",1)
        return bytes.fromhex(p).decode() if hmac.compare_digest(s,hmac.new(SECRET,p.encode(),hashlib.sha256).hexdigest()) else None
    except (ValueError,UnicodeDecodeError): return None

def public_user(r):
    return {"id":r["id"],"username":r["username"],"display_name":r["display_name"],"bio":r["bio"],"followers":r["followers"],"following":r["following"]}
...
def init_db():
    with db() as c:
        ...
        if not c.execute("SELECT 1 FROM users LIMIT 1").fetchone():
            c.execute("INSERT INTO users(id,username,password_hash,display_name,bio) VALUES(?,?,?,?,?)",("demo-user","reelo_creator",hash_password("demo"),"REelo Creator","Create. Watch. Connect."))
...
def update_me(data:ProfileUpdate,authorization:str|None=Header(default=None)):
    ...
    with db() as c:
        ...
        display=(data.display_name if data.display_name is not None else r["display_name"]).strip()
        bio=(data.bio if data.bio is not None else r["bio"]).strip()
        if not display or len(display)>80 or len(bio)>160: raise HTTPException(400,"Invalid profile data")
        c.execute("UPDATE users SET display_name=?,bio=? WHERE id=?",(display,bio,uid))
        r=c.execute("SELECT * FROM users WHERE id=?",(uid,)).fetchone()
...
def like(video_id:str,authorization:str|None=Header(default=None)):
    ...
    with db() as c:
        ...
        if x:
            ...
            add_notification(c,video["user_id"],uid,"like",video_id)
...
def comment(video_id:str,data:CommentIn,authorization:str|None=Header(default=None)):
    ...
    with db() as c:
        video=c.execute("SELECT user_id FROM videos WHERE id=?",(video_id,)).fetchone()
        if not video: raise HTTPException(404,"Video not found")
        cid=uuid4().hex; c.execute("INSERT INTO comments(id,user_id,video_id,body) VALUES(?,?,?,?)",(cid,uid,video_id,body)); c.execute("UPDATE videos SET comments=comments+1 WHERE id=?",(video_id,)); add_notification(c,video["user_id"],uid,"comment",video_id)
...
def follow(user_id:str,authorization:str|None=Header(default=None)):
    ...
    with db() as c:
        ...
        if x:
            c.execute("DELETE FROM follows WHERE follower_id=? AND following_id=?",(uid,user_id)); a="unfollow"; c.execute("UPDATE users SET followers=MAX(followers-1,0) WHERE id=?",(user_id,)); c.execute("UPDATE users SET following=MAX(following-1,0) WHERE id=?",(uid,))
        else:
            c.execute("INSERT INTO follows(follower_id,following_id) VALUES(?,?)",(uid,user_id)); a="follow"; c.execute("UPDATE users SET followers=followers+1 WHERE id=?",(user_id,)); c.execute("UPDATE users SET following=following+1 WHERE id=?",(uid,)); add_notification(c,user_id,uid,"follow")
    return {"action":a}
...
def notifications(limit:int=50,offset:int=0,authorization:str|None=Header(default=None)):
    ...
    limit=max(1,min(limit,100)); offset=max(0,offset)
    with db() as c:
        rows=c.execute("SELECT n.id,n.type,n.video_id,n.read,n.created_at,u.id actor_id,u.username actor_username,u.display_name actor_display_name FROM notifications n JOIN users u ON u.id=n.actor_id WHERE n.recipient_id=? ORDER BY n.created_at DESC LIMIT ? OFFSET ?",(uid,limit,offset)).fetchall()
        unread=c.execute("SELECT COUNT(*) FROM notifications WHERE recipient_id=? AND read=0",(uid,)).fetchone()[0]
        total=c.execute("SELECT COUNT(*) FROM notifications WHERE recipient_id=?",(uid,)).fetchone()[0]
    return {"items":[dict(r) for r in rows],"unread":unread,"total":total,"limit":limit,"offset":offset}
