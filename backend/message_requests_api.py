...
from main import app, current_user, db
...
def request_payload(r): return dict(r)

@router.get("")
def list_requests(kind:str="inbox",limit:int=50,offset:int=0,authorization:str|None=Header(default=None)):
    uid=require_user(authorization); kind=kind.strip().lower()
    if kind not in {"inbox","sent"}: raise HTTPException(400,"kind must be inbox or sent")
...
def request_summary(authorization:str|None=Header(default=None)):
    ...
    with db() as c:
        ensure_table(c); incoming=c.execute("SELECT COUNT(*) FROM message_requests WHERE recipient_id=? AND status='pending'",(uid,)).fetchone()[0]; outgoing=c.execute("SELECT COUNT(*) FROM message_requests WHERE requester_id=? AND status='pending'",(uid,)).fetchone()[0]
...
def accept_request(request_id:str,authorization:str|None=Header(default=None)):
    ...
    with db() as c:
        ...
        mid=uuid4().hex; c.execute("INSERT INTO messages(id,conversation_id,sender_id,body,delivered_at) VALUES(?,?,?, ?,CURRENT_TIMESTAMP)",(mid,cid,r["requester_id"],r["body"])); c.execute("UPDATE conversations SET updated_at=CURRENT_TIMESTAMP WHERE id=?",(cid,)); c.execute("UPDATE message_requests SET status='accepted',updated_at=CURRENT_TIMESTAMP WHERE id=?",(request_id,))
...
def decline_request(request_id:str,authorization:str|None=Header(default=None)):
    uid=require_user(authorization)
    with db() as c:
        ensure_table(c); cur=c.execute("UPDATE message_requests SET status='declined',updated_at=CURRENT_TIMESTAMP WHERE id=? AND recipient_id=? AND status='pending'",(request_id,uid))
...
def delete_request(request_id:str,authorization:str|None=Header(default=None)):
    uid=require_user(authorization)
    with db() as c:
        ensure_table(c); cur=c.execute("UPDATE message_requests SET status='declined',updated_at=CURRENT_TIMESTAMP WHERE id=? AND ((recipient_id=? AND status='pending') OR (requester_id=? AND status='pending'))",(request_id,uid,uid))
        if not cur.rowcount: raise HTTPException(404,"Message request not found")
...
def block_request(request_id:str,authorization:str|None=Header(default=None)):
    uid=require_user(authorization)
    with db() as c:
        ensure_table(c); r=c.execute("SELECT requester_id FROM message_requests WHERE id=? AND recipient_id=? AND status='pending'",(request_id,uid)).fetchone()
        if not r: raise HTTPException(404,"Message request not found")
        c.execute("CREATE TABLE IF NOT EXISTS blocked_users(blocker_id TEXT NOT NULL,blocked_id TEXT NOT NULL,created_at TEXT DEFAULT CURRENT_TIMESTAMP,PRIMARY KEY(blocker_id,blocked_id))"); c.execute("INSERT OR IGNORE INTO blocked_users(blocker_id,blocked_id) VALUES(?,?)",(uid,r["requester_id"])); c.execute("UPDATE message_requests SET status='blocked',updated_at=CURRENT_TIMESTAMP WHERE id=?",(request_id,))
