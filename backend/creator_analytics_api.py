...
def period_metrics(c, uid, days):
    ...
    d = dict(row)
    d['interactions'] = sum(int(d[k] or 0) for k in ('likes','comments','shares','saves'))
...
def overview(authorization: str | None = Header(default=None)):
    ...
    seconds = float(watch['seconds'] or 0); views = int(totals['views'] or 0)
    return {"videos":int(totals['videos'] or 0),"views":views,"likes":int(totals['likes'] or 0),"comments":int(totals['comments'] or 0),"engagement_rate":engagement(totals['likes'],totals['comments'],views),"watch_time_seconds":round(seconds,2),"watch_time_hours":round(seconds/3600,2),"average_watch_seconds":round(seconds/max(int(watch['events'] or 0),1),2),"unique_viewers":int(watch['unique_viewers'] or 0),"follower_total":int(followers[0]['followers']) if followers else 0,"follower_growth":(int(followers[0]['followers'])-int(followers[1]['followers'])) if len(followers)>1 else 0,"top_videos":[dict(x) for x in top],"retention_note":"Retention percentages require video-duration or completion telemetry; REelo does not invent them from incomplete data."}
...
def audience(authorization:str|None=Header(default=None)):
    uid=uid_or_401(authorization)
    with db() as c:
        ensure_followers(c,uid);rows=c.execute("SELECT day,followers FROM creator_follower_history WHERE creator_id=? ORDER BY day ASC LIMIT 90",(uid,)).fetchall();viewers=c.execute("SELECT COUNT(DISTINCT e.user_id) FROM events e JOIN videos v ON v.id=e.video_id WHERE v.user_id=? AND e.user_id!='anonymous'",(uid,)).fetchone()[0]
    items=[dict(x) for x in rows]
    for i,item in enumerate(items): item['growth']=int(item['followers'])-int(items[i-1]['followers']) if i else 0
...
def retention(authorization:str|None=Header(default=None)):
    uid=uid_or_401(authorization)
    with db() as c: rows=c.execute("SELECT CASE WHEN e.seconds < 3 THEN '0-3s' WHEN e.seconds < 10 THEN '3-10s' WHEN e.seconds < 30 THEN '10-30s' WHEN e.seconds < 60 THEN '30-60s' ELSE '60s+' END bucket,COUNT(*) viewers,COALESCE(SUM(e.seconds),0) seconds FROM events e JOIN videos v ON v.id=e.video_id WHERE v.user_id=? AND e.action IN ('view','watch','progress','complete') GROUP BY bucket ORDER BY CASE bucket WHEN '0-3s' THEN 1 WHEN '3-10s' THEN 2 WHEN '10-30s' THEN 3 WHEN '30-60s' THEN 4 ELSE 5 END",(uid,)).fetchall()
...
def comparison(authorization:str|None=Header(default=None)):
    uid=uid_or_401(authorization)
    with db() as c: seven=period_metrics(c,uid,7);thirty=period_metrics(c,uid,30)
    def daily_delta(key):
        a=float(seven[key] or 0)/7;b=float(thirty[key] or 0)/30
        return round(((a-b)/b)*100,1) if b else None
    return {"seven_day":seven,"thirty_day":thirty,"changes_vs_30d_daily_average":{k:daily_delta(k) for k in ('views','interactions','watch_seconds','unique_viewers')},"note":"7D values are compared with the 30D daily average so the different window lengths are normalized."}


@router.get("/video-performance")
def video_performance(authorization:str|None=Header(default=None)):
    uid=uid_or_401(authorization)
    with db() as c: rows=c.execute("SELECT v.id,v.caption,v.created_at,v.views,v.likes,v.comments,COALESCE((SELECT SUM(e.seconds) FROM events e WHERE e.video_id=v.id AND e.action IN ('view','watch','progress','complete')),0) watch_seconds,COALESCE((SELECT COUNT(DISTINCT e.user_id) FROM events e WHERE e.video_id=v.id AND e.user_id!='anonymous' AND e.action='view'),0) unique_viewers FROM videos v WHERE v.user_id=? ORDER BY v.created_at DESC LIMIT 50",(uid,)).fetchall()
    items=[]
    for r in rows:
        v=dict(r);views=int(v['views'] or 0);rate=engagement(v['likes'],v['comments'],views);avg=round(float(v['watch_seconds'] or 0)/max(views,1),2);score=round(min(100,rate*8+min(40,views/10)+min(20,avg*2)),1);v.update(interaction_rate=rate,avg_watch_seconds=avg,performance_score=score,score_note='Relative score using observed views, like/comment rate and average watch seconds; not a prediction.');items.append(v)
    return {"items":items}
...
def viewer_cohorts(authorization:str|None=Header(default=None)):
    uid=uid_or_401(authorization)
    with db() as c: rows=c.execute("SELECT e.user_id,MIN(e.created_at) first_seen,MAX(e.created_at) last_seen,COUNT(CASE WHEN e.action='view' THEN 1 END) views FROM events e JOIN videos v ON v.id=e.video_id WHERE v.user_id=? AND e.user_id!='anonymous' GROUP BY e.user_id",(uid,)).fetchall()
    from datetime import datetime,timedelta
    cutoff=(datetime.utcnow()-timedelta(days=30)).strftime('%Y-%m-%d');new=sum(1 for r in rows if str(r['first_seen'])[:10]>=cutoff);returning=sum(1 for r in rows if str(r['first_seen'])[:10]<cutoff)
...
def content_performance(authorization:str|None=Header(default=None)):
    uid=uid_or_401(authorization)
    with db() as c: rows=c.execute("SELECT id,caption,views,likes,comments,created_at FROM videos WHERE user_id=? ORDER BY created_at DESC LIMIT 100",(uid,)).fetchall()
    stats={}
    for r in rows:
        for token in set(theme_tokens(r['caption'])):
            s=stats.setdefault(token,{'theme':token,'videos':0,'views':0,'likes':0,'comments':0});s['videos']+=1;s['views']+=int(r['views'] or 0);s['likes']+=int(r['likes'] or 0);s['comments']+=int(r['comments'] or 0)
    items=[]
    for s in stats.values(): s['interaction_rate']=engagement(s['likes'],s['comments'],s['views']);items.append(s)
...
def posting_heatmap(authorization:str|None=Header(default=None)):
    uid=uid_or_401(authorization)
    with db() as c: rows=c.execute("SELECT CAST(strftime('%w',e.created_at) AS INTEGER) weekday,CAST(strftime('%H',e.created_at) AS INTEGER) hour,COUNT(CASE WHEN e.action='view' THEN 1 END) views,COUNT(CASE WHEN e.action IN ('like','comment','share','save') THEN 1 END) interactions FROM events e JOIN videos v ON v.id=e.video_id WHERE v.user_id=? GROUP BY weekday,hour",(uid,)).fetchall()
...
def next_content(authorization:str|None=Header(default=None)):
    uid=uid_or_401(authorization)
    with db() as c:
        posting=c.execute("SELECT CAST(strftime('%w',e.created_at) AS INTEGER) weekday,CAST(strftime('%H',e.created_at) AS INTEGER) hour,COUNT(CASE WHEN e.action='view' THEN 1 END) views,COUNT(CASE WHEN e.action IN ('like','comment','share','save') THEN 1 END) interactions FROM events e JOIN videos v ON v.id=e.video_id WHERE v.user_id=? GROUP BY weekday,hour HAVING views>0 ORDER BY views DESC,interactions DESC LIMIT 3",(uid,)).fetchall();top=c.execute("SELECT caption,views,likes,comments FROM videos WHERE user_id=? ORDER BY views DESC,likes DESC LIMIT 10",(uid,)).fetchall()
    themes=Counter(t for r in top for t in theme_tokens(r['caption']));rec=[]
...
def creator_intelligence(authorization:str|None=Header(default=None)):
    uid=uid_or_401(authorization)
    with db() as c:
        posting=c.execute("SELECT CAST(strftime('%w',e.created_at) AS INTEGER) weekday,CAST(strftime('%H',e.created_at) AS INTEGER) hour,COUNT(CASE WHEN e.action='view' THEN 1 END) views,COUNT(CASE WHEN e.action IN ('like','comment','share','save') THEN 1 END) interactions,COALESCE(SUM(e.seconds),0) watch_seconds FROM events e JOIN videos v ON v.id=e.video_id WHERE v.user_id=? GROUP BY weekday,hour HAVING views>0 ORDER BY views DESC,interactions DESC LIMIT 12",(uid,)).fetchall();videos=c.execute("SELECT v.id,v.caption,v.created_at,v.views,v.likes,v.comments FROM videos v WHERE v.user_id=? ORDER BY v.views DESC,v.likes DESC LIMIT 20",(uid,)).fetchall();event_count=c.execute("SELECT COUNT(*) FROM events e JOIN videos v ON v.id=e.video_id WHERE v.user_id=?",(uid,)).fetchone()[0]
    ...
    if videos:
        scored=[{**dict(v),'interaction_rate':engagement(v['likes'],v['comments'],v['views'])} for v in videos];reach=max(scored,key=lambda x:x['views']);eng=max(scored,key=lambda x:(x['interaction_rate'],x['views']));insights.append({"type":"content","title":"Best engagement signal","detail":"Strongest observed like/comment rate among recent videos.","video_id":eng['id'],"rate":eng['interaction_rate']});insights.append({"type":"content","title":"Best reach","detail":"Highest-viewed recent video.","video_id":reach['id'],"views":int(reach['views'] or 0)})
